    ...
    try {
      if (req.user) {
        ...
        if (user.activation) {
          ...
          return next(new ApiError(`Your account isn't Actived  ! `, 403));
        }
      } else {
      ...
      if (req.user) {
        ...
        if (residentail) {
          if (
            ...
            return next(
              new ApiError(`Sorry You Can't view This List As Owner`, 403)
            );
          }
        } else if (costal) {
          if (
  ...
  verifyToken(req, res, async () => {
    try {
      if (req.user) {
        ...
        return next(new ApiError(`You are not authenticated! `, 40));
      }
    } catch (error) {
      return next(new ApiError(`System Error ${error}`, 400));
    }
