        ...
        if (user.activation) {
          ...
          if (user.credits >= credits) {
            if (req.body.typeOfRequest == "residential") {
              ...
              return next(new ApiError(`there are error in the data `, 400));
            }
          } else {
            return next(
              new ApiError(`Sorry you don't have Credits enough `, 402 )
            );
          }
        } else {
          return next(new ApiError(`Your account isn't Activited ! `, 403 ));
        }
      } else {
  ...
  verifyToken(req, res, async () => {
    try {
      if (req.user) {
        ...
        const result = paginate(AllRent, limit, page);
        res.status(200).json({ Rents: result });
      } else {
        return next(new ApiError(`You are not authenticated! `, 401));
      }
    } catch (error) {
      return next(new ApiError(`Ststem Error ${error}`, 400));
    }
  });
};
      ...
      if (req.user) {
        ...
        if (residentail) {
          if (
            ...
            return next(new ApiError(`You Can't Update This request `, 403));
          }
        } else if (costal) {
          if (
      ...
      if (req.user) {
        ...
        if (residentail) {
          if (
            ...
            return next(new ApiError(`You Can't Delete This Request `, 403));
          }
        } else if (costal) {
          if (
            req.user.id == costal.userId ||
      ...
      if (req.user) {
        ...
        if (residentail) {
          if (
            ...
            return next(
              new ApiError(`Sorry You Can't view This request As Owner`, 403)
            );
          }
        } else if (costal) {
          if (
