...
export const filteration = async (req, res, next) => {
  try {
    verifyToken(req, res, async () => {
      if (req.user) {
        const {minRange,maxRange,maxArea,minArea,keyWords,...otherQueryParams } = req.query;
        ...
        if (otherQuery.typeOfList == "residential") {
          ...
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,residentialList)) 
          ...
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,commercialList)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfList == "costal") {
          const lists = await costalList.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,costalList)) 
          res.status(200).json({Lists : Listing})
        }
      } else {
    ...
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        if (otherQuery.typeOfList == "residential") {
          ...
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,residentialRent)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfList == "commercial") {
          const lists = await commercialRent.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,commercialRent)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfList == "costal") {
          const lists = await costalRent.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,costalRent)) 
          res.status(200).json({Lists : Listing})
        }
      } else {
        return next(new ApiError(`You are not authenticated! `, 404));
    ...
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        if (otherQuery.typeOfRequest == "residential") {
          ...
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,residentailRequest)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfRequest == "commercial") {
          const lists = await commercialRequest.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,commercialRequest)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfRequest == "costal") {
          const lists = await costalRequest.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,costalRequest)) 
          res.status(200).json({Lists : Listing})
        }
      } else {
        return next(new ApiError(`You are not authenticated! `, 404));
    ...
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        if (otherQuery.typeOfRequest == "residential") {
          ...
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,residentialRequestRent)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfRequest == "commercial") {
          const lists = await commercialRequestRent.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,commercialRequestRent)) 
          res.status(200).json({Lists : Listing})
        } else if (otherQuery.typeOfRequest == "costal") {
          const lists = await costalRequestRent.find(otherQuery);
          const Listing = await(Search(minRange , maxRange , minArea , maxArea , keyWords , lists ,costalRequestRent)) 
          res.status(200).json({Lists : Listing})
        }
      } else {
        return next(new ApiError(`You are not authenticated! `, 404));
