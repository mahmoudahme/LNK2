  ...
  try {
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        await incrementView(AllListing , residentailRequest , commercialRequest , costalRequest)
  ...
  try {
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        let Request = shuffleArray(Requests);
        sort(Request)
        await incrementViewAll(Request , residentailRequest , residentailRequestRent)
        res.status(200).json({ All: Request });
      } else {
        return next(new ApiError(`You are not authenticated! `, 404));
      }
  ...
  try {
    verifyToken(req, res, async () => {
      if (req.user) {
        ...
        let Request = shuffleArray(Requests);
        sort(Request)
        await incrementViewAll(Request ,commercialRequest, commercialRequestRent )
